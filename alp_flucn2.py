...
b0=2*xch*sqrt(1-xch**2)#(xch+xch**2)*(1-xch**2)**(0.5)
a0=0.0*sqrt(1-xch**2)*(1+0.001*xch) #(1.-.1*xch*sqrt(3.*pi/4.))*
#a0r=dot(mbp,b0)
b0r=dot(df1r,a0)
hel=zeros(Nch)
hels=zeros(Nch)
...
for i in range(Nc):
    ...
    #for i in range(2):
    #bet=1./(1+abs(b0)**2)
    D1b=tau*diag(Calpha*Cmu*alff+Ch*hels);
...
levh=-max(abs(hS).flatten())/2.+max(abs(hS).flatten())*arange(12)/11.
levb=-max(abs(bt0).flatten())/2.+max(abs(bt0).flatten())*arange(12)/11.
leva=-max(abs(brrt).flatten())/2.+max(abs(brrt).flatten())*arange(12)/11.
#levb2=-max(abs(b2).flatten())/2.+max(abs(b2).flatten())*arange(12)/11.
plt.figure(figsize=[8,4])
CS1=plt.contour(xt[::4,:],yt[::4,:],bt0[::4,:], levb,colors='k')
CS2=plt.pcolor(xt[::4,:], yt[::4,:], brrt[::4,:],cmap=cmap.seismic,vmin=-.1,vmax=.1)
...
CB.set_label('Br',fontsize=30)
plt.subplots_adjust(hspace=0,bottom=0.1, left=0.1, right=0.84, top=0.95)
plt.show()
...
plt.figure(figsize=[8,4])
CS1=plt.contour(xt[::4,:],yt[::4,:],bt0[::4,:], levb,colors='k')
CS2=plt.pcolor(xt[::4,:], yt[::4,:], (hl0+abM)[::4,:],norm=colors.SymLogNorm(linthresh=.001, linscale=10,
...
divider = make_axes_locatable(plt.gca())
cax = divider.append_axes("right", "3%", pad="3%")
CB=plt.colorbar(CS2, cax=cax,ticks=[-0.005,0,0.005],format='%.2f')
...
HC=zeros((Nc,Nch),float);
hcm=zeros((Nc,Nch),float);
...
for i in range(0,Nc):
    ...
    hLU=-br0*a0*Rw*(1.-xch**2/4.)
    hla=-(bp*afm-ap*bfm)*tensordot(xch,ones(Nph),0)
    
    hlb=Rbu*(ap*bp+afm*bfm)*tensordot(vbu(b0),ones(Nph),0)
    hlB=Rbu*(a0*b0)*vbu(b0)
    hc=br*curlr
    ...
    HC[i,:]=sum(hc,axis=1)*2*pi/Nph
    hcm[i,:]=sum(brn*curlr,axis=1)*2*pi/Nph
    if remainder(i,1000)==0:
        print(i)
...
plt.figure(figsize=[8,4])
CS1=plt.contour(xt[::4,:],yt[::4,:],bt0[::4,:], levb,colors='k')
CS2=plt.pcolor(xt[::4,:], yt[::4,:], (hl0+abM)[::4,:],norm=colors.SymLogNorm(linthresh=.01, linscale=10,
            vmin=-.01,vmax=.01),cmap=cmap.seismic)
#№,cmap=cmap.seismic,vmin=-.1,vmax=.1)
#plt.xlabel('R$^2/\\eta_T$')
plt.ylabel('LATITUDE')
...
CB.set_label('$\\langle{\chi}\\rangle$',fontsize=30)
plt.subplots_adjust(hspace=0,bottom=0.1, left=0.1, right=0.84, top=0.95)
plt.show()
# In[16]:
...
plt.figure(figsize=[8,4])
#CS1=plt.contour(xt[::4,:],yt[::4,:],bt0[::4,:], levb,colors='k')
CS2=plt.pcolor(xt[::4,:], yt[::4,:], 100*(abM)[::4,:],norm=colors.SymLogNorm(linthresh=.01, linscale=10,
            vmin=-.01,vmax=.01),cmap=cmap.seismic)
#№,cmap=cmap.seismic,vmin=-.1,vmax=.1)
#plt.xlabel('R$^2/\\eta_T$')
plt.ylabel('LATITUDE')
...
cax = divider.append_axes("right", "3%", pad="3%")
CB=plt.colorbar(CS2, cax=cax,ticks=[-0.01,0,0.01],format='%.2f')
CB.set_label('100$\\tilde{\chi}$',fontsize=30)
plt.subplots_adjust(hspace=0,bottom=0.1, left=0.1, right=0.84, top=0.95)
plt.show()
# In[16]:
...
plt.figure(figsize=[8,4])
#CS1=plt.contour(xt[::4,:],yt[::4,:],bt0[::4,:], levb,colors='k')
CS2=plt.pcolor(xt[::4,:], yt[::4,:], (hcm)[::4,:],norm=colors.SymLogNorm(linthresh=.01, linscale=1,
...
plt.figure(figsize=[8,4])
#CS1=plt.contour(xt[::4,:],yt[::4,:],bt0[::4,:], levb,colors='k')
CS2=plt.pcolor(xt[::4,:], yt[::4,:], (abBU+abbu)[::4,:],norm=colors.SymLogNorm(linthresh=.01, linscale=1,
...
divider = make_axes_locatable(plt.gca())
cax = divider.append_axes("right", "3%", pad="3%")
CB=plt.colorbar(CS2, cax=cax,ticks=[-0.1,-0.02,0.02,0.1],format='%.2f')
...
plt.figure(figsize=[8,4])
#CS1=plt.contour(xt[::4,:],yt[::4,:],bt0[::4,:], levb,colors='k')
CS2=plt.pcolor(xt[::4,:], yt[::4,:], 100*(abbu+aba)[::4,:],norm=colors.SymLogNorm(linthresh=.01, linscale=1,
            vmin=-.05,vmax=.05),cmap=cmap.seismic)
#№,cmap=cmap.seismic,vmin=-.1,vmax=.1)
#plt.xlabel('R$^2/\\eta_T$')
plt.ylabel('LATITUDE')
...
divider = make_axes_locatable(plt.gca())
cax = divider.append_axes("right", "3%", pad="3%")
CB=plt.colorbar(CS2, cax=cax,ticks=[-0.04,-0.01,0.01,.04],format='%.2f')
CB.set_label('100(F$_{\\alpha}$+F$_{\\beta}$)',fontsize=30)
plt.subplots_adjust(hspace=0,bottom=0.1, left=0.1, right=0.84, top=0.95)
plt.show()
# In[16]:
plt.figure(figsize=[8,4])
#CS1=plt.contour(xt[::4,:],yt[::4,:],bt0[::4,:], levb,colors='k')
CS2=plt.pcolor(xt[:,:], yt[:,:], 1e6*flap[:,:],norm=colors.SymLogNorm(linthresh=.01, linscale=10,
...
# In[16]:
plt.figure()
plt.plot(timi,sar,'k',lw=1.8)
plt.xlim( (0.3,tr[-1]))
plt.show()

plt.figure()
plt.plot(sar,'k',lw=1.8)
#plt.xlim( (0.3,tr[-1]))
plt.show()
# In[16]:
plt.figure()
plt.plot(timi,dot((abdr+aba+abbu)[:,:Nch//2],wch[:Nch//2]),'r',
         timi,dot((abdr+aba+abbu)[:,Nch//2:],wch[Nch//2:]),'b',
         timi,dot((abDR+abBU)[:,:Nch//2],wch[:Nch//2]),'r--',
         timi,dot((abDR+abBU)[:,Nch//2:],wch[Nch//2:]),'b--',
         lw=1.8)
plt.xlim( (0.3,tr[-1]))
plt.show()
...
plt.plot(timi,dot((abdr+aba+abbu)[:,:Nch//2],wch[:Nch//2]),'r',
         timi,dot((abdr+aba+abbu)[:,Nch//2:],wch[Nch//2:]),'b',
         timi,-Rw*dot((brrt*at0)[:,:Nch//2],(0.-xch[:Nch//2]**2/4)*wch[:Nch//2]),'r--',
         timi,-Rw*dot((brrt*at0)[:,Nch//2:],(0.-xch[Nch//2:]**2/4)*wch[Nch//2:]),'b--',
         lw=1.8)
#plt.xlim( (0.3,tr[-1]))
plt.show()
...
cilm[1,nk,mk]=imag(bfc) 
bfcr=SHctor(cilm)
abf= MakeGridGLQ(bfcr,zero,norm=4)[-1::-1,:]+tensordot(abM[-1674,:],ones(Nph),0)#+tensordot(ABM[-1674,:],ones(Nph),0)

levs=linspace(-1.,1.,15)

plt.figure()
CS2=plt.pcolor(xa,ya,abf,norm=colors.SymLogNorm(linthresh=.01, linscale=10,
            vmin=-.02,vmax=.02),cmap=cmap.seismic)
#plt.contour(xa,ya,bf[:,:],levs,colors='k')
divider = make_axes_locatable(plt.gca())
cax = divider.append_axes("right", "3%", pad="3%")
...
ax.contour(xn,yn, bffc,levs/10,colors='k',transform=ccrs.PlateCarree())
CS=ax.pcolormesh(xn,yn,brfc,cmap=cmap.seismic, vmin=-0.1,vmax=0.1, transform=ccrs.PlateCarree())
CB=plt.colorbar(CS,orientation='horizontal',fraction=0.05, pad=0.02)#, cax=cax)
#CB.set_label('B$_r$',fontsize=24)
#plt.tight_layout() 
plt.show()
# In[13]:

brfc = add_cyclic_point(abf)
plt.figure()
ax = plt.axes(projection=ccrs.Mollweide(central_longitude=90))
#ax = plt.axes(projection=ccrs.Orthographic(central_latitude=20,central_longitude=180.0-(phi[1]-phi[0])*180/pi/2.))
#ax.contour(xn,yn, bffc,levs,colors='k',transform=ccrs.PlateCarree())
CS=ax.pcolormesh(xn,yn,brfc,norm=colors.SymLogNorm(linthresh=.003, linscale=10,
            vmin=-.01,vmax=.01),cmap=cmap.seismic, transform=ccrs.PlateCarree())
CB=plt.colorbar(CS,orientation='horizontal',fraction=0.05, pad=0.02,
                ticks=[-0.01,0.,.01],format='%.2f')#, cax=cax)
#CB.set_label('B$_r$',fontsize=24)
#plt.tight_layout() 
plt.show()
# In[13]:
...
plt.figure()
plt.plot(xch,mean(br,axis=1),'k',xch,b0+mean(bf,axis=1),'k--')
plt.show()    

# In[13]:

plt.figure()
plt.plot(xch,100*mean(abf,axis=1),'k',xch,ABM[-1674,:],'k--',
         xch,hl0[-1674,:],'r')
plt.show()    

# In[13]:

brfc = add_cyclic_point(hlu+hla-hlb)
plt.figure()
ax = plt.axes(projection=ccrs.Mollweide(central_longitude=90))
#ax = plt.axes(projection=ccrs.Orthographic(central_latitude=20,central_longitude=180.0-(phi[1]-phi[0])*180/pi/2.))
#ax.contour(xn,yn, bffc,levs,colors='k',transform=ccrs.PlateCarree())
CS=ax.pcolormesh(xn,yn,brfc,norm=colors.SymLogNorm(linthresh=.01, linscale=10,
            vmin=-.1,vmax=.1),cmap=cmap.seismic, transform=ccrs.PlateCarree())
CB=plt.colorbar(CS,orientation='horizontal',fraction=0.05, pad=0.02,
                ticks=[-0.05,0.,.05],format='%.2f')#, cax=cax)
#CB.set_label('B$_r$',fontsize=24)
#plt.tight_layout() 
plt.show()
# In[13]:
...
plt.figure()
plt.plot(xch,mean((hlu+hla-hlb),axis=1),'k')
plt.show()    
